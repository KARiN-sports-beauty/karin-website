"""health 7件だけ Embedding を生成・保存する。

正本 health_knowledge_data.py と DB 本文が一致していること。
① official / ② notes は更新しない。OpenAI は Embedding 生成時のみ。

  python scripts/embed_health_knowledge.py --dry-run
  python scripts/embed_health_knowledge.py --apply
"""
...
from health_knowledge_data import SOURCE_KEYS, iter_knowledge_payloads  # noqa: E402
...
def load_health_rows(rows: list[dict]) -> tuple[dict[str, dict], list[str]]:
    ...
    for row in rows:
        ...
        if key not in SOURCE_KEYS:
            continue
        if row.get("source_type") != HEALTH:
            mismatches.append(f"{key}: source_type={row.get('source_type')}（health であるべき）")
            continue
        payload = payloads[key]
        if (row.get("content") or "") != payload["content"]:
            mismatches.append(f"{key}: DB本文が health_knowledge_data.py と一致しない")
            continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="③ health 7件だけ Embedding する")
    ...
    before_official_n, before_official_hash = fingerprint(official_active, OFFICIAL)
    before_notes_n, before_notes_hash = fingerprint(rows, NOTES, set(NOTES_KEYS))
    found, mismatches = load_health_rows(rows)
    ...
    print("official after:", after_official_n, "hash:", after_official_hash[:16])
    print("notes after:", after_notes_n, "hash:", after_notes_hash[:16])
    ...
    health_after = {
        ...
        for r in after_rows
        if r.get("source_type") == HEALTH and r.get("source_key") in SOURCE_KEYS
    }
    for key in SOURCE_KEYS:
        row = health_after.get(key)
        dim = embedding_len(row.get("embedding") if row else None)
        if dim != EMBEDDING_DIMS:
            print("error: embedding 未保存または次元不正:", key, dim)
            return 1
    print("official 19件 / notes 6件: 変更なし")
