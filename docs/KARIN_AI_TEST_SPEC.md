...
期待：固定のフォーム案内文。「お問い合わせフォームへ」CTA。遷移先 `/contact`。予約API・LLM・RAGを呼ばない。

## TRAINER-02

上記のあと「ツアー帯同」「10/3〜25」などが続いても、ヒアリングを始めない。手配・依頼承知の表現を出さない。
...
上記のあと日付・人数・実施内容が続いても、訪問確定・手配の表現を出さない。
