...
期待：初回予約案内（ヘッダーの『ご予約』、Web予約へ進む、このまま私との会話でご予約をお取りしたい場合、エリアと施術時間）。`show_booking_cta: true`。直後に「ご希望のエリアと施術時間を教えてください。例：東京・90分」を重複表示しない。
...
期待：`guest_last_name` / `guest_first_name` に自動分割して保存しない。姓と名を分けて確認する。予約は作成しない。

## BOOK-GUEST-02

入力：「姓：藤田、名：幸士、090-1234-5678、test@example.com、東京都渋谷区○○」（出張）

期待：4項目を一度に保存し、追加質問せず最終確認へ進む。その「はい」で既存の予約確定処理へ進む。空白区切りの「藤田 幸士」も姓名として認識する。
